...
// ─── DATABASE MIGRATION ────────────────────────────────────────────────────────
//
//  Run this ONCE in your Supabase SQL Editor before deploying this version:
//
...
//
//  (Optional FK references — recommended for data integrity)
...
//
//  NOTE: Enable Email Confirmations in your Supabase project:
...
app.post('/api/transactions', requireAuth, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('transactions')
            .insert([{ account_id, type, amount: parseFloat(amount), description, user_id: uid }])
            .select();
        if (error) throw error;
