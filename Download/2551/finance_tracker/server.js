...
// GET /api/liabilities — return all liabilities ordered by creation date
app.get('/api/liabilities', async (req, res) => {
    try {
        ...
        console.error('Liabilities GET Error:', e);
...
app.post('/api/liabilities', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            .from('liabilities')
            .insert([{ name: name.trim(), amount: parseFloat(amount) }])
            .select();

        if (error) throw error;
        res.status(201).json(data[0]);
    } catch (e) {
        console.error('Liabilities POST Error:', e);
        res.status(500).json({ error: e.message || 'Failed to add liability' });
    }
});

// DELETE /api/liabilities/:id — remove a liability
app.delete('/api/liabilities/:id', async (req, res) => {
    try {
        ...
        console.error('Liabilities DELETE Error:', e);
